...
Allows converting Azure EventHub events to Syslog messages and sending them to a RELP server.
...
* Allows the usage of certificates from Azure KeyVault for TLS encryption.
...
The project must be deployed as a Function in a Function App. The Function App requires a few App Settings to function
properly.


|===
|App Setting |Description |Default value |Possible values
